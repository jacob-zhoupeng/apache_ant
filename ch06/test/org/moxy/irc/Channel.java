...
/**
 * A repersentation of a channel (or room) on IRC.
 * This class is part of the IRC framework.
 * In general NO set methods should be called;
 * all that is handled by IRCConnection. The set methods may
 * be set more private later.
 * <br><br>
 * The original version of this class was made by
 * Hex-binary (http://members.xoom.com/hexbinary) for hIRC.<br>
 * I have removed the GUI portion of this class for
 * use in Oak.
 * @version 1.0
 * @author Hex-binary modified by Marcus Wenzel
 * @author Christiaan Welvaart
 */
/*/
 * Repersents a IRC channel
...
public class Channel {
	...
	private final static int MODE_TOPIC = 0;
	private final static int MODE_NOEXTMSG = 1;
	private final static int MODE_SECRET = 2;
	private final static int MODE_INVITE = 3;
	private final static int MODE_PRIVATE = 4;
	private final static int MODE_MODERATED = 5;
	private final static int MODE_LIMIT = 6;
	private final static int MODE_KEY = 7;
...
//    private String url;
	...
	{
//		System.out.print("Channel::findNick: looking for " + name + " in ");
//		for (int j = 0; j < nicks.size(); j++)
//			System.out.print(getNickAt(j).getName() + " ");
//		System.out.println();
		
		int i;
		
		for (i = 0; i < nicks.size(); i++)
			if (getNickAt(i).getName().equalsIgnoreCase(name))
				return getNickAt(i);
    ...
    void setInviteOnly(boolean b, String chanop) {
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setInviteOnly(b, chanop);
    }
     /**
    ...
    void setPrivate(boolean b, String chanop) {
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setPrivate(b, chanop);
    }
    /**
    ...
    void setSecret(boolean b, String chanop) {
        modes[MODE_SECRET] = b;
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setSecret(b, chanop);
    }
     /**
    ...
    void setModerated(boolean b, String chanop) {
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setModerated(b, chanop);
    }
    /**
    ...
    void setNoExtMsg(boolean b, String chanop) {
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setNoExtMsg(b, chanop);
    }
    /**
    ...
    void setOpTopic(boolean b, String chanop) {
        modes[MODE_TOPIC] = b;
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setOpTopic(b, chanop);
    }
    /**
    ...
    void setLimit(int limit, String chanop) {
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setLimit(limit, chanop);
    }
    /**
    ...
    void ban(String mask, boolean mode, String chanop) {
		if (!mode)
			ban_list.removeElement(mask);
		else
        	ban_list.addElement(mask);
		for(int x = 0; x<listeners.length; x++)
         listeners[x].ban(mask, mode, chanop);
    }
    ...
    void join(String nick) {
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].join(name, ident, host);
    }
    ...
    void part(String name, String ident, String host, String msg) {
        if (!removeNickByName(name))
			return;
		
		for(int x = 0; x<listeners.length; x++)
         listeners[x].part(name, ident, host, msg);
    }
    ...
    void quit(String name, String ident, String host, String msg) {
        if (!removeNickByName(name))
			return;
		for(int x = 0; x<listeners.length; x++)
         listeners[x].quit(name, ident, host, msg);
    }
    ...
    void nickChange(String oldName, String newName) {
		...
		if (org.moxy.oak.Preferences.DEBUG_LEVEL >= 20)
			System.out.println("Channel::nickChange for " + name + ": " + oldName + " -> " + newName);
        if (!isHere(oldName))
		{
         	int i;
			StringBuffer buf;
			
			if (org.moxy.oak.Preferences.DEBUG_LEVEL >= 15)
			{
				System.out.println("Channel::nickChange: nick not in channel!");
				buf = new StringBuffer("users on channel " + name + ": ");
				for (i = 0; i < nicks.size(); i++)
				{
					buf.append(getNameAt(i));
		    		buf.append(" ");
				}
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].nickChange(oldName,newName);
    }
    /**
       * Removes a nick from the channel.
       * @since 1.0
    ...
    void kick(String name, String reason, String chanop) {
        if (!removeNickByName(name))
			return;
		for(int x = 0; x<listeners.length; x++)
         listeners[x].kick(name, reason, chanop);
    }
    ...
    void op(String name, boolean mode, String chanop) {
		...
		nick.setChanop(mode);
 		...
 		for(int x = 0; x<listeners.length; x++)
         listeners[x].op(name, mode, chanop);
    }
    ...
    void voice(String name, boolean mode, String chanop) {
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].voice(name, mode, chanop);
    }
	...
	void handleMsg(String sender, IRCLine message)
	{
		...
		for(i = 0; i<listeners.length; i++)
			listeners[i].handleMessage(sender, message);
	}
	...
	void handleCTCPMsg(String sender, IRCLine message)
	{
		...
		if (command.equals("ACTION"))
			for(i = 0; i<listeners.length; i++)
				listeners[i].handleAction(sender, message);
	}
	...
	void initialNickListComplete()
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialNickList(nicks);
	}
	...
	void initialTopic(String topic)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialTopic(topic);
	}
	...
	void initialOpTopic(boolean mode)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialOpTopic(mode);
	}
	...
	void initialNoExtMsg(boolean mode)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialNoExtMsg(mode);
	}
	...
	void initialSecret(boolean mode)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialSecret(mode);
	}
	...
	void initialInviteOnly(boolean mode)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialInviteOnly(mode);
	}
	...
	void initialPrivate(boolean mode)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialPrivate(mode);
	}
	...
	void initialModerated(boolean mode)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialModerated(mode);
	}
	...
	void initialLimit(boolean mode, int limit)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialLimit(mode, limit);
	}
	...
	void initialKey(boolean mode, String key)
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialKey(mode, key);
	}
	...
	void initialBanListComplete()
	{
		...
		for (i = 0; i < listeners.length; i++)
			listeners[i].initialBan(ban_list);
	}
    ...
    public Vector getOpList() {
		...
		for (i = 0; i < nicks.size(); i++)
		{
			nick = getNickAt(i);
			if (nick.isChanop())
				opList.addElement(nick.getName());
		}
    ...
    public Vector getVoiceList() {
		...
		for (i = 0; i < nicks.size(); i++)
		{
			nick = getNickAt(i);
			if (nick.hasVoice())
				voiceList.addElement(nick.getName());
		}
    ...
    public Vector getRegList() {
		...
		{
			nick = getNickAt(i);
			if (!nick.isChanop() && !nick.hasVoice())
				regList.addElement(nick.getName());
		}
    ...
    public void setOtherMode(char mode, boolean type, String chanop) {
		...
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setOtherMode(mode, type, chanop);
    }
    ...
    public void setTopic(String topic, String chanop) {
        this.topic = topic;
		for(int x = 0; x<listeners.length; x++)
         listeners[x].setTopic(topic, chanop);
    }
...
//    }
	...
	public void addListener(ChannelListener cl)
	{
		...
		for(int x = 0; x<listeners.length; x++)
			newlisteners[x] = listeners[x];
		newlisteners[newlisteners.length-1] = cl;
		listeners = null;
		listeners = newlisteners;

		if (ban_list_complete)
			cl.initialBan(ban_list);
		if (nick_list_complete)
			cl.initialNickList(nicks);
		
		if (!topic.equals(""))
			cl.initialTopic(topic);
		if (modes[0])
			cl.initialOpTopic(true);
		if (modes[1])
			cl.initialNoExtMsg(true);
		if (modes[2])
			cl.initialSecret(true);
		if (modes[3])
			cl.initialInviteOnly(true);
		if (modes[4])
			cl.initialPrivate(true);
		if (modes[5])
			cl.initialModerated(true);
		if (modes[6])
			cl.initialLimit(true, limitValue);
		if (modes[7])
			cl.initialKey(true, keyValue);
	}
	...
	public void removeListener(ChannelListener cl)
	{
		...
		for(int x = 0; x<index; x++)
			newlisteners[x] = listeners[x];
		for(int x = index+1; x<listeners.length; x++)
			newlisteners[x-1] = listeners[x];
		listeners = null;
		listeners = newlisteners;
	}
