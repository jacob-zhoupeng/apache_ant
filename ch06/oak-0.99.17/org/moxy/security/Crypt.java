/*
 * A method to generate a random seed then encrypt the password along 
 * with a method to check an encrypted password with the original
 * password was added by Marcus Wenzel moxy@moxy.org
 */
 ...
 * Saved about 6-9% in class file size, 1% speed gain
...
public class Crypt {
  ...
  /**
   * Checks that the encrypted password and the unencrypted password match.
   * @param encrypted the encrypted password with the salt.
   * @param password the unencrypted password to check against.
   */
  public static final boolean check(String encrypted, String password){
   String salt = encrypted.substring(0,2);
   password = crypt(salt,password);
   return encrypted.equals(password);
  }
  /**
   * Generates a random salt then encrypts.
   * Created by <a href="mailto:moxy@moxy.org">Marcus Wenzel</a>
   */
